...
/*   Updated: 2019/01/05 01:11:36 by gvirga           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	uim_add_managment(char **str, t_args *s, t_type *px, int len)
{
	(*s).len = ft_strlen(*str);
	while (len >= 0)
	{
	...
	(*str)[0] = '0';
	(*str)[1] = 'x';
	if (*str && !((*s).f & 4) && ((*s).fy > (*s).len))
	{
		*str = !((*s).f & 2) ?
			ft_strjoin_free(calc_space_width((*s).fy, (*s).len), *str, 3) :
			ft_strjoin_free(*str, calc_space_width((*s).fy, (*s).len), 3);
	}
}
